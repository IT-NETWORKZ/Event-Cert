...
const AddEvent = () => {
  ...
  const [eventsList, setEventsList] = useState(initialDummyEvents);
  ...
  const eventColumns = [
    {
      header: "Sr No",
      accessorKey: "id", // Changed from id to accessorKey to match row access strategies
    ...
    {
      header: "Action",
      id: "actions", // Keep pure ID here because it acts as an unmapped column layout
      meta: { className: "text-center" },
      cell: (info) => (
        <button
          type="button"
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: "30px" }}>
        <DataTable columns={eventColumns} data={eventsList} />
      </div>
    </div>
  );
};
